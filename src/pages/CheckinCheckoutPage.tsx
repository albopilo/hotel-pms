...
import { formatIDR, formatDate, formatTime, todayISO, addDays, nightsBetween, isEarlyCheckin, isLateCheckout, formatHoursShort } from '@/lib/format';
import { getLockProvider } from '@/lib/hotel-lock/provider';
import { LogIn, LogOut, KeyRound, AlertCircle, CheckCircle2, Loader2, BedDouble } from 'lucide-react';
import type { Reservation, Guest, Room, RoomType, Folio, BookingSource, Branch, ChargeCategory, PaymentMethod } from '@/types/database';
...
export function CheckinCheckoutPage({ initialReservationId, searchQuery }: { initialReservationId?: string | null; searchQuery?: string }) {
  ...
  useEffect(() => {
    if (initialReservationId) {
      ...
      if (res) { setSelected(res); setMode(res.status === 'checked_in' ? 'checkout' : 'checkin'); }
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title={`${t('dash.arrivals_today')} / ${t('res.confirmed')}`}>
          {arrivals.filter(filterFn).length === 0 ? (
            <EmptyState title={t('common.no_data')} />
          ) : (
            <div className="space-y-2">
              {arrivals.filter(filterFn).map((r) => {
                const g = guestMap.get(r.primary_guest_id || '');
                const rm = roomMap.get(r.room_id || '');
                return (
                  <div key={r.id} className="flex items-center justify-between border border-slate-100 rounded-lg px-3 py-2 hover:bg-slate-50">
                    <div>
        ...
        <Card title={`${t('dash.departures_today')} / ${t('res.checked_in')}`}>
          {departures.filter(filterFn).length === 0 ? (
            <EmptyState title={t('common.no_data')} />
          ) : (
            <div className="space-y-2">
              {departures.filter(filterFn).map((r) => {
                const g = guestMap.get(r.primary_guest_id || '');
                const rm = roomMap.get(r.room_id || '');
                return (
                  <div key={r.id} className="flex items-center justify-between border border-slate-100 rounded-lg px-3 py-2 hover:bg-slate-50">
      ...
      {selected && mode === 'checkin' && (
        <CheckinModal reservation={selected} onClose={() => { setSelected(null); setMode(null); load(); }} />
      )}
      {selected && mode === 'checkout' && (
        <CheckoutModal reservation={selected} onClose={() => { setSelected(null); setMode(null); load(); }} />
      )}
    </div>
  );
}
...
function CheckinModal({ reservation, onClose }: { reservation: Reservation; onClose: () => void }) {
  ...
  const handleAddEarlyCharge = async () => {
    if (!folio) return;
    const { data: settings } = await supabase.from('system_settings').select('value').eq('key', 'early_checkin_charge').maybeSingle();
    const charge = parseFloat(settings?.value || '0');
    if (charge <= 0) { showToast('No early check-in charge configured', 'warning'); return; }
    await supabase.from('folio_items').insert({
      ...
      quantity: 1, unit_amount: charge, amount: charge, business_date: todayISO(),
      created_by: user!.id,
    });
    await supabase.from('transactions').insert({
      ...
      transaction_type: 'early_checkin_charge', description: 'Early check-in charge', amount: charge,
      debit_credit: 'debit', business_date: todayISO(), created_by: user!.id,
    });
  ...
  const encodeCard = async () => {
    ...
    if (!connected) { setCardState('unavailable'); setCardMessage(t('checkin.lock_unavailable')); return; }
    setCardState('writing'); setCardMessage(t('checkin.writing'));
    ...
    if (result.success) {
      setCardState('confirming'); setCardMessage(t('checkin.confirming'));
      await new Promise((r) => setTimeout(r, 500));
      setCardState('success'); setCardMessage(t('checkin.card_success'));
      await supabase.from('card_issuances').insert({
        branch_id: reservation.branch_id, reservation_id: reservation.id, guest_id: guest.id,
        room_id: room.id, issuance_type: 'issue', card_sequence: 1,
      ...
      setCardState('failed'); setCardMessage(result.message);
      await supabase.from('card_issuances').insert({
        branch_id: reservation.branch_id, reservation_id: reservation.id, guest_id: guest.id,
        room_id: room.id, issuance_type: 'issue', card_sequence: 1,
        valid_from: `${reservation.check_in_date}T${checkinTime}`,
  ...
  const completeCheckin = async () => {
    ...
    const { error } = await supabase.from('reservations').update({
      status: 'checked_in', actual_check_in: now, check_in_time: checkinTime,
    }).eq('id', reservation.id);
    if (error) { showToast(error.message, 'error'); setCompleting(false); return; }
    if (room) await supabase.from('rooms').update({ status: 'occupied' }).eq('id', room.id);
    await supabase.from('audit_logs').insert({
      organization_id: user!.organization_id, branch_id: reservation.branch_id, user_id: user!.id,
      action: 'check_in', object_type: 'reservation', object_id: reservation.id,
      ...
      <div className="space-y-4">
        ...
        <div className="border border-slate-200 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2"><KeyRound size={18} className="text-slate-600" /><span className="font-medium text-slate-700">{t('checkin.encode_card')}</span></div>
            <span className="text-xs text-amber-600 font-medium bg-amber-50 px-2 py-0.5 rounded">DEVELOPMENT / MOCK MODE</span>
          </div>
          {room && guest && (
            <div className="text-sm text-slate-500 mb-3">
              {t('common.room')}: <span className="font-medium">{room.room_number}</span> · {t('common.guest')}: <span className="font-medium">{guest.full_name}</span>
            </div>
          )}
          {cardState === 'idle' && <Button onClick={encodeCard} disabled={!room}><KeyRound size={16} /> {t('checkin.encode_card')}</Button>}
          {cardState !== 'idle' && (
            <div className="flex items-center gap-3">
              {(cardState === 'connecting' || cardState === 'writing' || cardState === 'confirming') && <Loader2 size={20} className="animate-spin text-blue-600" />}
              {cardState === 'success' && <CheckCircle2 size={20} className="text-emerald-600" />}
              {cardState === 'failed' && <AlertCircle size={20} className="text-red-600" />}
              {cardState === 'unavailable' && <AlertCircle size={20} className="text-amber-600" />}
              <span className={`text-sm font-medium ${cardState === 'success' ? 'text-emerald-700' : cardState === 'failed' ? 'text-red-700' : cardState === 'unavailable' ? 'text-amber-700' : 'text-slate-600'}`}>
                {cardMessage}
              </span>
            </div>
          )}
...
function CheckoutModal({ reservation, onClose }: { reservation: Reservation; onClose: () => void }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setGuest(g as Guest); setRoom(r as Room); setFolio(f as Folio);
      if (f) {
  ...
  const balance = (totalCharges + totalTax - totalDiscounts) - totalPayments + (reservation.deposit > 0 ? -reservation.deposit : 0);
  const hasUnpaid = balance > 0;
  const canOverride = user?.role === 'super_admin' || user?.role === 'manager';

  const handleAddLateCharge = async () => {
    if (!folio) return;
    const { data: settings } = await supabase.from('system_settings').select('value').eq('key', 'late_checkout_charge').maybeSingle();
    const charge = parseFloat(settings?.value || '0');
    if (charge <= 0) { showToast('No late checkout charge configured', 'warning'); return; }
    await supabase.from('folio_items').insert({
      folio_id: folio.id, branch_id: reservation.branch_id, reservation_id: reservation.id,
      guest_id: reservation.primary_guest_id, room_id: reservation.room_id,
  ...
  const completeCheckout = async () => {
    ...
    const { error } = await supabase.from('reservations').update({
      status: 'checked_out', actual_check_out: now, check_out_time: checkoutTime,
    }).eq('id', reservation.id);
    if (error) { showToast(error.message, 'error'); setCompleting(false); return; }
    if (room) await supabase.from('rooms').update({ status: 'dirty' }).eq('id', room.id);
    if (folio) await supabase.from('folios').update({ status: 'finalized', finalized_at: new Date().toISOString(), finalized_by: user!.id }).eq('id', folio.id);
    // Invalidate card
    const provider = getLockProvider();
  ...
  return (
    <Modal open onClose={onClose} title={t('checkout.title')} size="lg">
      <div className="space-y-4">
        ...
        <div className="border border-slate-200 rounded-lg overflow-hidden">
          <table className="w-full text-sm">
            <tbody>
              ...
              <tr className="border-b border-slate-100"><td className="py-2 px-3 text-slate-500">{t('checkout.additional_charges')}</td><td className="text-right py-2 px-3 font-medium">{formatIDR(charges.filter((i) => !['room', 'amenity', 'early_checkin', 'late_checkout', 'damage'].includes(i.category)).reduce((s, i) => s + i.amount, 0))}</td></tr>
              ...
              <tr className="border-b border-slate-100"><td className="py-2 px-3 text-slate-500">{t('checkout.damage_charges')}</td><td className="text-right py-2 px-3 font-medium">{formatIDR(charges.filter((i) => i.category === 'damage').reduce((s, i) => s + i.amount, 0))}</td></tr>
              ...
              <tr className="border-b border-slate-100"><td className="py-2 px-3 text-slate-500">{t('common.deposit')}</td><td className="text-right py-2 px-3 font-medium text-emerald-600">{formatIDR(reservation.deposit)}</td></tr>
        ...
        {overrideUnpaid && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-amber-700 text-sm">Override applied — checkout will proceed with unpaid balance (logged to audit).</div>
        )}
