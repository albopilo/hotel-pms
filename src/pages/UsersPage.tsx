...
(map[a.user_id]||(map[a.user_id]=[])).push(a.branch_id);
});
setAccessMap(map);
setLoading(false);
},[]);
...
{profiles.length===0?<EmptyState icon={<Users size={48}/>} title={t('common.no_data')}/>:<Card noPadding>
...
<tbody>{profiles.map(p=><tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50">
...
<td className="py-3 px-4 text-slate-500">{p.role==='super_admin'?t('common.all_branches'):(accessMap[p.id]||[]).map(id=>branches.find(b=>b.id===id)?.name).filter(Boolean).join(', ')||'-'}</td>
...
<td className="text-right py-3 px-4"><div className="flex justify-end gap-2">
...
function UserFormModal({open,onClose,profile,branches,currentAccess,orgId,onSaved}:{open:boolean;onClose:()=>void;profile:Profile|null;branches:Branch[];currentAccess:string[];orgId:string;onSaved:()=>void}){
const {t}=useI18n();
const {showToast}=useToast();
...
try{
if(profile){
const {error}=await supabase.from('profiles').update({full_name:form.full_name,role:form.role,is_active:form.is_active}).eq('id',profile.id);
if(error)throw error;
await supabase.from('user_branch_access').delete().eq('user_id',profile.id);
if(form.role!=='super_admin'&&form.branchIds.length)await supabase.from('user_branch_access').insert(form.branchIds.map(branch_id=>({user_id:profile.id,branch_id})));
}else{
const {data,error}=await supabase.functions.invoke('create-user',{
body:{email:form.email,password:form.password,full_name:form.full_name,role:form.role,organization_id:orgId,branchIds:form.branchIds}
});
if(error)throw error;
}
...
{form.role!=='super_admin'&&<div>
<label className="text-sm font-medium block mb-2">{t('common.branch')} Access</label>
<div className="space-y-2 max-h-48 overflow-y-auto border rounded-lg p-3">
{branches.map(b=><label key={b.id} className="flex gap-2 text-sm">
<input type="checkbox" checked={form.branchIds.includes(b.id)} onChange={e=>setForm({...form,branchIds:e.target.checked?[...form.branchIds,b.id]:form.branchIds.filter(id=>id!==b.id)})}/>
{b.name}
</label>)}
</div>
</div>}
