...
export function ReservationFormModal({ open, onClose, branches, rooms, roomTypes, guests, bookingSources, userId, orgId, defaultBranchId, reservation, onSaved }: {
  ...
  return (
    ...
    // Sync reservation charges (room, discount, tax, deposit) to folio
    const targetRes = newRes || reservation;
    if (targetRes) {
      const { data: folio } = await supabase
        .from('folios')
        .select('id')
        .eq('reservation_id', targetRes.id)
        .maybeSingle();
      if (folio) {
        await folioService.syncReservationChargesToFolio(folio.id, targetRes as Reservation, userId);
      }
    }
